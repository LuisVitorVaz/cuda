...
__global__ void somaVetor(int *vetor, int *resultado) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x; /// numero de thread = numeros de blocos * (numero de thread por bloco )+ id da thread;
...
int main() {
    ...
    somaVetor<<<2, 512>>>(d_vetor, d_resultado);//<<<1,1>>> primeiro o bloco depois a thread
